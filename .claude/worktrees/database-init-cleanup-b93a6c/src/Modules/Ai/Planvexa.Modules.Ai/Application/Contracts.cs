...
public sealed record AiProviderSettingsDto(string BaseUrl, string Model, string ApiKeyMask, bool IsEnabled);

/// <summary>A null/blank <paramref name="ApiKey" /> keeps the stored key (write-only key).</summary>
public sealed record UpdateAiProviderSettingsCommand(string BaseUrl, string Model, string? ApiKey, bool IsEnabled);
