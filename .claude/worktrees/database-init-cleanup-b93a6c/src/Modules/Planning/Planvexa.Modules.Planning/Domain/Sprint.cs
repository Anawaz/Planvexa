...
public sealed class Sprint : Entity, IAggregateRoot, IWorkspaceOwned
{
    ...
    private Sprint(Guid id, Guid workspaceId, string name, DateTime startDate, DateTime endDate, Guid createdBy, DateTimeOffset nowUtc)
        : base(id)
    {
    ...
    public static Sprint Create(Guid id, Guid workspaceId, string name, DateTimeOffset startUtc, DateTimeOffset endUtc, Guid createdBy, DateTimeOffset nowUtc)
    {
        ...
        return new Sprint(id, workspaceId, name.Trim(), start, end, createdBy, nowUtc);
    }
    ...
    public void ChangeStatus(SprintStatus status) => Status = status;
