    ...
    private async Task<(bool Ok, string? Note)> ApplyAsync(AutomationAction action, WorkspaceEvent workspaceEvent, CancellationToken ct)
    {
        ...
        switch (action.Type)
        {
            ...
            case AutomationAction.Types.Integration:
                // STUB (see AutomationAction.Types.Integration doc comment): there is not yet
                // built real third-party integrations. Recorded as a no-op so run history/dry-run clearly
                // show the rule WOULD have called an integration here, once one exists to call.
                return (false, "integration action is an extension point; not yet implemented");
