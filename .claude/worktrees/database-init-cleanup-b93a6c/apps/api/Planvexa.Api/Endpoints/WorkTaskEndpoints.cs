...
public static class WorkTaskEndpoints
{
    ...
    private static void MapTasks(RouteGroupBuilder api)
    {
        ...
        tasks.MapGet("/mine", async (WorkItemService svc, CancellationToken ct) =>
        ...
        api.MapGet("/lists/{listId:guid}/tasks", async (Guid listId, WorkItemService svc, CancellationToken ct) =>
            Results.Ok(await svc.ListByListAsync(listId, ct)))
            .RequireAuthorization()
            .RequireOAuthScope(OAuthScopes.TasksRead);
