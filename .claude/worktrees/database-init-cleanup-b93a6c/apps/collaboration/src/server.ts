...
type RoomContext = { userId: string | null; workspaceId: string };
...
function isWhiteboardRoom(documentName: string): boolean {
  return documentName.startsWith(WHITEBOARD_PREFIX);
}

function resourceId(documentName: string): string {
...
const server = new Server({
  ...
  extensions: [
    new Database({
      fetch: async ({ documentName }) =>
        isWhiteboardRoom(documentName) ? fetchWhiteboardState(resourceId(documentName)) : fetchState(documentName),
      store: async ({ documentName, state, lastContext }) => {
        const workspaceId = (lastContext as Partial<RoomContext> | undefined)?.workspaceId;
        if (!workspaceId) {
  ...
  async onAuthenticate(data) {
    ...
    const context: RoomContext = { userId: access.userId, workspaceId };
    return context;
  },
});
