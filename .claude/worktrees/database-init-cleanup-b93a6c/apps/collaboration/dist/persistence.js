 ...
 */
const pool = new Pool({
    connectionString: process.env.PLANVEXA_COLLAB_DB_URL ??
        "postgresql://planvexa:planvexa@localhost:5432/planvexa",
});
export async function fetchState(documentId) {
    const result = await pool.query("SELECT y_state FROM docs.document_collab_state WHERE document_id = $1", [documentId]);
    const row = result.rows[0];
    return row ? new Uint8Array(row.y_state) : null;
}
export async function storeState(documentId, workspaceId, state) {
    await pool.query(`INSERT INTO docs.document_collab_state (document_id, workspace_id, y_state, updated_at_utc)
     VALUES ($1, $2, $3, now())
     ON CONFLICT (document_id)
     DO UPDATE SET y_state = excluded.y_state, updated_at_utc = excluded.updated_at_utc`, [documentId, workspaceId, Buffer.from(state)]);
}
/**
 ...
 */
export async function fetchWhiteboardState(whiteboardId) {
    const result = await pool.query("SELECT y_state FROM whiteboards.whiteboard_collab_state WHERE whiteboard_id = $1", [whiteboardId]);
    const row = result.rows[0];
    return row ? new Uint8Array(row.y_state) : null;
}
export async function storeWhiteboardState(whiteboardId, workspaceId, state) {
    await pool.query(`INSERT INTO whiteboards.whiteboard_collab_state (whiteboard_id, workspace_id, y_state, updated_at_utc)
     VALUES ($1, $2, $3, now())
     ON CONFLICT (whiteboard_id)
     DO UPDATE SET y_state = excluded.y_state, updated_at_utc = excluded.updated_at_utc`, [whiteboardId, workspaceId, Buffer.from(state)]);
}
