...
public sealed class ResourcePermissionFlowTests(PlanvexaFixture fixture)
{
    ...
    [Fact]
    public async Task Listing_a_list_with_mixed_private_and_public_tasks_returns_exactly_what_the_caller_can_read()
    {
        // Regression guard for the batched authorization rewrite of WorkItemService.ListByListAsync
        // (was an N-round-trip-per-task loop): the VISIBLE SET must be identical to what the old
        // per-task CanReadInListContextAsync loop would have produced, for a list that mixes ordinary
        // tasks, an individually-private task, and an individually-ACL-granted task.
        var (ownerClient, workspaceId, _, _) = await fixture.NewWorkspaceClientAsync();
        var space = await ownerClient.CreateSpaceAsync("Team Space");
        var list = await ownerClient.CreateListAsync(space.Id, "Mixed List");

        var publicTaskA = await ownerClient.CreateTaskAsync(list.Id, "Public A");
        var publicTaskB = await ownerClient.CreateTaskAsync(list.Id, "Public B");
        var privateTask = await ownerClient.CreateTaskAsync(list.Id, "Private task");
        var grantedTask = await ownerClient.CreateTaskAsync(list.Id, "Individually granted task");

        var (memberSubject, memberUserId) = await fixture.InviteMemberAsync(ownerClient, workspaceId, "member");
        var memberClient = fixture.WorkClient(memberSubject, workspaceId);

        // Baseline: every task is visible before any privacy/ACL feature is used.
        (await memberClient.GetFromJsonAsync<List<TaskResp>>($"/api/v1/lists/{list.Id}/tasks"))!
            .Select(t => t.Id).ShouldBe(
                [publicTaskA.Id, publicTaskB.Id, privateTask.Id, grantedTask.Id], ignoreOrder: true);

        (await ownerClient.PatchAsJsonAsync($"/api/v1/resources/task/{privateTask.Id}/private", new { isPrivate = true }))
            .StatusCode.ShouldBe(HttpStatusCode.OK);

        // A grant directly on a non-private task exercises the "individually ACL-gated" path (the task
        // itself is not private, but IResourcePermissionQuery.HasAnyGrantAsync/the batched equivalent
        // must still route it through the full resolver instead of the cheap coarse-role fallback).
        (await ownerClient.PostAsJsonAsync(
                $"/api/v1/resources/task/{grantedTask.Id}/permissions",
                new { principalType = "user", principalId = memberUserId, level = "view" }))
            .StatusCode.ShouldBe(HttpStatusCode.Created);

        var visible = (await memberClient.GetFromJsonAsync<List<TaskResp>>($"/api/v1/lists/{list.Id}/tasks"))!
            .Select(t => t.Id).ToList();

        // The private task with no grant is filtered out; the two ordinary public tasks and the
        // individually-granted task all remain visible.
        visible.ShouldBe([publicTaskA.Id, publicTaskB.Id, grantedTask.Id], ignoreOrder: true);
        visible.ShouldNotContain(privateTask.Id);

        // The owner (Admin+, coarse role) still sees every task, private one included.
        (await ownerClient.GetFromJsonAsync<List<TaskResp>>($"/api/v1/lists/{list.Id}/tasks"))!
            .Select(t => t.Id).ShouldBe(
                [publicTaskA.Id, publicTaskB.Id, privateTask.Id, grantedTask.Id], ignoreOrder: true);
    }

    [Fact]
    public async Task Task_listing_pagination_returns_a_stable_non_overlapping_page_sequence_covering_every_task()
    {
        var (ownerClient, _, _, _) = await fixture.NewWorkspaceClientAsync();
        var space = await ownerClient.CreateSpaceAsync("Team Space");
        var list = await ownerClient.CreateListAsync(space.Id, "Paged List");

        var created = new List<Guid>();
        for (var i = 0; i < 5; i++)
        {
            created.Add((await ownerClient.CreateTaskAsync(list.Id, $"Task {i}")).Id);
        }

        var fullOrder = (await ownerClient.GetFromJsonAsync<List<TaskResp>>($"/api/v1/lists/{list.Id}/tasks"))!
            .Select(t => t.Id).ToList();
        fullOrder.ShouldBe(created, ignoreOrder: true);

        // Walk the list two at a time via the `after`/`limit` keyset cursor and confirm the pages are
        // non-overlapping, cover every task exactly once, and reproduce the same order as the unpaginated
        // call -- i.e. paging never skips or duplicates a task.
        var paged = new List<Guid>();
        Guid? after = null;
        for (var guard = 0; guard < created.Count + 1; guard++)
        {
            var url = after is { } cursor
                ? $"/api/v1/lists/{list.Id}/tasks?after={cursor}&limit=2"
                : $"/api/v1/lists/{list.Id}/tasks?limit=2";
            var page = (await ownerClient.GetFromJsonAsync<List<TaskResp>>(url))!;
            if (page.Count == 0)
            {
                break;
            }

            paged.AddRange(page.Select(t => t.Id));
            after = page[^1].Id;
        }

        paged.ShouldBe(fullOrder);
        paged.Distinct().Count().ShouldBe(fullOrder.Count);
    }
