...
internal sealed record AiFeatureStatusResp(bool Enabled);
...
public sealed class AiExpansionFlowTests(PlanvexaFixture fixture)
{
    ...
    /// <summary>
    /// The "allow AI to be completely disabled" master switch (AiProviderSettings.AiFeaturesEnabled):
    /// with it off, every AI action is rejected with 403 before it runs, regardless of the offline/real
    /// provider routing; the default (never toggled) keeps every existing AI capability working exactly as
    /// before, and only an Admin+ can flip the switch.
    /// </summary>
    [Fact]
    public async Task Disabling_ai_features_blocks_every_ai_action_and_re_enabling_restores_them()
    {
        var (owner, workspaceId, slug, _) = await fixture.NewWorkspaceClientAsync();
        var space = await owner.CreateSpaceAsync("AI toggle space");
        var list = await owner.CreateListAsync(space.Id, "AI toggle list");
        var task = await owner.CreateTaskAsync(list.Id, "AI toggle task");

        // Default: AI features are enabled for a workspace that never touched the switch.
        (await owner.GetFromJsonAsync<AiFeatureStatusResp>("/api/v1/ai/features"))!.Enabled.ShouldBeTrue();

        var (memberSubject, _) = await fixture.InviteMemberAsync(owner, workspaceId, "ai-toggle-member");
        var memberClient = fixture.WorkClient(memberSubject, slug, workspaceId);
        (await memberClient.PutAsJsonAsync("/api/v1/ai/settings/features", new { enabled = false }))
            .StatusCode.ShouldBe(HttpStatusCode.Forbidden);

        var disable = await owner.PutAsJsonAsync("/api/v1/ai/settings/features", new { enabled = false });
        disable.StatusCode.ShouldBe(HttpStatusCode.OK);
        (await owner.GetFromJsonAsync<AiFeatureStatusResp>("/api/v1/ai/features"))!.Enabled.ShouldBeFalse();

        foreach (var (name, call) in AiActionCalls(task.Id))
        {
            (await call(owner)).StatusCode.ShouldBe(HttpStatusCode.Forbidden, name);
        }

        var enable = await owner.PutAsJsonAsync("/api/v1/ai/settings/features", new { enabled = true });
        enable.StatusCode.ShouldBe(HttpStatusCode.OK);
        (await owner.GetFromJsonAsync<AiFeatureStatusResp>("/api/v1/ai/features"))!.Enabled.ShouldBeTrue();

        foreach (var (name, call) in AiActionCalls(task.Id))
        {
            (await call(owner)).StatusCode.ShouldBe(HttpStatusCode.OK, name);
        }
    }

    /// <summary>Every AI action the toggle must gate (summarize, subtasks, priority, risk, duplicates, ask,
    /// semantic search) as a single request each, driven from the same task/workspace.</summary>
    private static List<(string Name, Func<HttpClient, Task<HttpResponseMessage>> Call)> AiActionCalls(Guid taskId) =>
    [
        ("summarize", c => c.PostAsync(new Uri($"/api/v1/ai/tasks/{taskId}/summarize", UriKind.Relative), null)),
        ("subtasks", c => c.PostAsync(new Uri($"/api/v1/ai/tasks/{taskId}/subtasks", UriKind.Relative), null)),
        ("priority", c => c.PostAsync(new Uri($"/api/v1/ai/tasks/{taskId}/priority", UriKind.Relative), null)),
        ("risk", c => c.PostAsync(new Uri($"/api/v1/ai/tasks/{taskId}/risk", UriKind.Relative), null)),
        ("duplicates", c => c.PostAsync(new Uri($"/api/v1/ai/tasks/{taskId}/duplicates", UriKind.Relative), null)),
        ("ask", c => c.PostAsJsonAsync("/api/v1/ai/ask", new { question = "What tasks are open?" })),
        ("semantic search", c => c.GetAsync(new Uri("/api/v1/ai/search/semantic?q=task", UriKind.Relative))),
    ];
