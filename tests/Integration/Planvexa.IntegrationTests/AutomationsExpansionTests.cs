...
internal sealed record TeamFullResp(Guid Id, Guid WorkspaceId, string Name, string? Description, bool IsArchived, int MemberCount);
...
public sealed class AutomationsExpansionTests(PlanvexaFixture fixture)
{
    ...
    // ---- UI-shaped condition keys match the real event vocabulary ----

    [Fact]
    public async Task Rule_built_with_the_automations_UIs_exact_condition_shape_fires_on_a_real_status_change()
    {
        // AutomationsPageClient.tsx's condition builder serializes {"field":"toStatusId","equals":"<id>"}
        // (see conditionJson()/conditionFields in that component) — this proves that exact payload shape,
        // built against a REAL WorkspaceEventTranslator-produced task.status_changed event (not a
        // synthetic dictionary), actually matches and fires. Guards against the field-name vocabulary
        // (toStatusId/assigneeUserId/listId, not status/assignee/list) silently drifting out of sync again.
        var (client, _, _, _) = await fixture.NewWorkspaceClientAsync();
        var space = await client.CreateSpaceAsync();
        var list = await client.CreateListAsync(space.Id);
        var task = await client.CreateTaskAsync(list.Id, "UI-shaped condition target");
        var schemes = await client.GetSchemesAsync();
        var targetStatus = schemes.Single(s => s.IsDefault).Statuses.First(s => s.Id != task.StatusId);
        var otherStatus = schemes.Single(s => s.IsDefault).Statuses.First(s => s.Id != task.StatusId && s.Id != targetStatus.Id);

        // Matching rule: only fires when toStatusId equals the exact status we are about to move to.
        var matchingRule = await client.PostAsJsonAsync("/api/v1/automations", new
        {
            name = "UI-shaped: matches target status",
            triggerType = "task.status_changed",
            conditionJson = $$"""{"field":"toStatusId","equals":"{{targetStatus.Id}}"}""",
            actionJson = """[{"type":"add_tag","value":"ui-shaped-match"}]""",
        });
        matchingRule.StatusCode.ShouldBe(HttpStatusCode.Created);
        var matchingRuleDto = (await matchingRule.Content.ReadFromJsonAsync<AutomationRuleFullResp>())!;

        // Control rule with the same UI shape but a DIFFERENT expected status id: proves the match is on
        // the real toStatusId value, not merely "the field key happens to exist".
        var nonMatchingRule = await client.PostAsJsonAsync("/api/v1/automations", new
        {
            name = "UI-shaped: does not match",
            triggerType = "task.status_changed",
            conditionJson = $$"""{"field":"toStatusId","equals":"{{otherStatus.Id}}"}""",
            actionJson = """[{"type":"add_tag","value":"ui-shaped-should-not-fire"}]""",
        });
        nonMatchingRule.StatusCode.ShouldBe(HttpStatusCode.Created);

        (await client.PatchAsJsonAsync($"/api/v1/tasks/{task.Id}", new { statusId = targetStatus.Id }))
            .StatusCode.ShouldBe(HttpStatusCode.OK);

        var runs = await PollAsync(async () =>
        {
            var found = await client.GetFromJsonAsync<List<AutomationRunFullResp>>($"/api/v1/automations/{matchingRuleDto.Id}/runs");
        ...
        var tags = await client.GetFromJsonAsync<List<TagResp>>("/api/v1/tags");
        tags!.ShouldContain(t => t.Name == "ui-shaped-match");
        tags!.ShouldNotContain(t => t.Name == "ui-shaped-should-not-fire");
    }
    ...
    [Fact]
    public async Task Integration_action_invokes_the_configured_slack_provider()
    {
        var (client, _, _, _) = await fixture.NewWorkspaceClientAsync();
        var space = await client.CreateSpaceAsync();
        var list = await client.CreateListAsync(space.Id);

        // Configure Slack with an unreachable webhook URL (same "prove it attempted a real call" shape as
        // Webhook_action_fires_an_ad_hoc_signed_call_through_the_integrations_pipeline) — this is the real
        // ISlackClient (Planvexa.Api.Integrations.SlackClient), not a fake, invoked through the "integration"
        // action's wiring (AutomationDispatcher.ApplyIntegrationAsync -> IIntegrationActionInvoker).
        var configure = await client.PutAsJsonAsync("/api/v1/integrations/providers/slack", new
        {
            configJson = "{}",
            secret = "http://127.0.0.1:59324/planvexa-unreachable-slack-webhook",
            isEnabled = true,
        });
        ...
        var createRule = await client.PostAsJsonAsync("/api/v1/automations", new
        {
            ...
            triggerType = "task.created",
            conditionJson = "{}",
            actionJson = """[{"type":"integration","value":"{\"provider\":\"slack\",\"message\":\"New task: {{task.title}}\"}"}]""",
        });
        createRule.StatusCode.ShouldBe(HttpStatusCode.Created);
        var rule = (await createRule.Content.ReadFromJsonAsync<AutomationRuleFullResp>())!;

        await client.CreateTaskAsync(list.Id, "Slack-notified task");
        ...
        runs.ShouldNotBeNull();
        runs!.ShouldContain(r => r.Detail != null && r.Detail.Contains("integration="));
    }

    [Fact]
    public async Task Integration_action_reports_a_clear_failure_for_a_provider_with_no_real_implementation()
    {
        var (client, _, _, _) = await fixture.NewWorkspaceClientAsync();
        var space = await client.CreateSpaceAsync();
        var list = await client.CreateListAsync(space.Id);

        // "gitlab" is settings-scaffolding only (IntegrationProviders.RealImplementation contains only
        // slack/github) -- the action must never fake a success for it, configured or not.
        var createRule = await client.PostAsJsonAsync("/api/v1/automations", new
        {
            name = "Notify GitLab",
            triggerType = "task.created",
            conditionJson = "{}",
            actionJson = """[{"type":"integration","value":"{\"provider\":\"gitlab\",\"message\":\"hi\"}"}]""",
        });
        createRule.StatusCode.ShouldBe(HttpStatusCode.Created);
        var rule = (await createRule.Content.ReadFromJsonAsync<AutomationRuleFullResp>())!;

        await client.CreateTaskAsync(list.Id, "Not GitLab-notified task");
        ...
        runs.ShouldNotBeNull();
        runs!.ShouldContain(r => r.Status == "Success" && r.Detail != null && r.Detail.Contains("integration=noop"));
    }
    ...
    // ---- New actions: set_priority/assign_team ----

    [Fact]
    public async Task Set_priority_action_updates_the_triggering_tasks_priority()
    {
        var (client, _, _, _) = await fixture.NewWorkspaceClientAsync();
        var space = await client.CreateSpaceAsync();
        var list = await client.CreateListAsync(space.Id);
        ...
        var createRule = await client.PostAsJsonAsync("/api/v1/automations", new
        {
            name = "Escalate to high",
            triggerType = "task.created",
            conditionJson = "{}",
            actionJson = """[{"type":"set_priority","value":"High"}]""",
        });
        createRule.StatusCode.ShouldBe(HttpStatusCode.Created);
        var rule = (await createRule.Content.ReadFromJsonAsync<AutomationRuleFullResp>())!;

        var task = await client.CreateTaskAsync(list.Id, "Needs escalation");
        ...
        runs.ShouldNotBeNull();
        runs!.ShouldContain(r => r.Status == "Success" && r.Detail != null && r.Detail.Contains("set_priority=ok"));

        // GET /api/v1/tasks/{id} returns an envelope ({ task, watcherUserIds, ... }), not a bare task DTO
        // — see TaskDetailEnvelope's doc comment in FormsCompletenessFlowTests.cs.
        var taskAfter = await PollAsync(async () =>
        {
            var found = await client.GetFromJsonAsync<TaskDetailEnvelope>($"/api/v1/tasks/{task.Id}");
            return found?.Task is { Priority: "High" } ? found : null;
        }, TimeSpan.FromSeconds(10));
        taskAfter.ShouldNotBeNull();
        taskAfter!.Task.Priority.ShouldBe("High");
    }

    [Fact]
    public async Task Assign_team_action_assigns_the_configured_team_to_the_triggering_task()
    {
        var (client, workspaceId, _, _) = await fixture.NewWorkspaceClientAsync();
        var space = await client.CreateSpaceAsync();
        var list = await client.CreateListAsync(space.Id);

        var createTeam = await client.PostAsJsonAsync($"/api/v1/workspaces/{workspaceId}/teams", new { name = "On-call", description = (string?)null });
        createTeam.StatusCode.ShouldBe(HttpStatusCode.Created);
        var team = (await createTeam.Content.ReadFromJsonAsync<TeamFullResp>())!;
        ...
        var createRule = await client.PostAsJsonAsync("/api/v1/automations", new
        {
            ...
            triggerType = "task.created",
            conditionJson = "{}",
            actionJson = $$"""[{"type":"assign_team","value":"{{team.Id}}"}]""",
        });
        createRule.StatusCode.ShouldBe(HttpStatusCode.Created);
        var rule = (await createRule.Content.ReadFromJsonAsync<AutomationRuleFullResp>())!;

        var task = await client.CreateTaskAsync(list.Id, "Needs a team");
        ...
        runs.ShouldNotBeNull();
        runs!.ShouldContain(r => r.Status == "Success" && r.Detail != null && r.Detail.Contains("assign_team=ok"));
        ...
        {
            var found = await client.GetFromJsonAsync<TaskDetailEnvelope>($"/api/v1/tasks/{task.Id}");
            return found?.Task.TeamAssigneeIds.Contains(team.Id) == true ? found : null;
        }, TimeSpan.FromSeconds(10));
        taskAfter.ShouldNotBeNull();
        taskAfter!.Task.TeamAssigneeIds.ShouldContain(team.Id);
    }
