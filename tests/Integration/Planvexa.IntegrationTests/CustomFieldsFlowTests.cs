...
public sealed class CustomFieldsFlowTests(PlanvexaFixture fixture)
{
    ...
    [Fact]
    public async Task Team_field_accepts_a_workspace_team_and_rejects_an_unknown_id()
    {
        var (client, workspaceId, _, _) = await fixture.NewWorkspaceClientAsync();
        var space = await client.CreateSpaceAsync();
        var list = await client.CreateListAsync(space.Id);
        ...
        var ok = await client.PutAsJsonAsync($"/api/v1/tasks/{task.Id}/custom-fields/{field.Id}", new { value = team.Id.ToString() });
        ok.StatusCode.ShouldBe(HttpStatusCode.OK);

        var unknown = await client.PutAsJsonAsync($"/api/v1/tasks/{task.Id}/custom-fields/{field.Id}", new { value = Guid.NewGuid().ToString() });
