...
internal sealed record CurrentUserResp(Guid UserId, string Email, string DisplayName, string? Timezone = null, string? Locale = null, string? Theme = null);
...
public sealed class UserDataFlowTests(PlanvexaFixture fixture)
{
    ...
    /// <summary>
    /// PATCH /api/v1/users/me — self-service profile rename. Verifies the change both in the PATCH
    /// response and in a subsequent GET, i.e. it actually persisted rather than just echoing the input.
    /// </summary>
    [Fact]
    public async Task Patch_me_updates_display_name_and_it_is_reflected_afterwards()
    {
        var (client, _, _, _) = await fixture.NewWorkspaceClientAsync();

        var patchResponse = await client.PatchAsJsonAsync(
            new Uri("/api/v1/users/me", UriKind.Relative), new { displayName = "Renamed Person" });
        patchResponse.StatusCode.ShouldBe(HttpStatusCode.OK);
        var patched = await patchResponse.Content.ReadFromJsonAsync<CurrentUserResp>();
        patched!.DisplayName.ShouldBe("Renamed Person");

        var getResponse = await client.GetFromJsonAsync<CurrentUserResp>("/api/v1/users/me");
        getResponse!.DisplayName.ShouldBe("Renamed Person");
        getResponse.UserId.ShouldBe(patched.UserId);
    }

    /// <summary>PATCH /api/v1/users/me also persists the timezone/locale display preferences, again
    /// verified via a follow-up GET (not just the PATCH echo).</summary>
    [Fact]
    public async Task Patch_me_persists_timezone_and_locale()
    {
        var (client, _, _, _) = await fixture.NewWorkspaceClientAsync();

        var patchResponse = await client.PatchAsJsonAsync(
            new Uri("/api/v1/users/me", UriKind.Relative),
            new { displayName = "Renamed Person", timezone = "Europe/Berlin", locale = "de-DE" });
        patchResponse.StatusCode.ShouldBe(HttpStatusCode.OK);
        var patched = await patchResponse.Content.ReadFromJsonAsync<CurrentUserResp>();
        patched!.Timezone.ShouldBe("Europe/Berlin");
        patched.Locale.ShouldBe("de-DE");

        var getResponse = await client.GetFromJsonAsync<CurrentUserResp>("/api/v1/users/me");
        getResponse!.Timezone.ShouldBe("Europe/Berlin");
        getResponse.Locale.ShouldBe("de-DE");
    }

    /// <summary>PATCH /api/v1/users/me also persists the theme preference (ThemeContext syncs
    /// from this on login) — verified via a follow-up GET, not just the PATCH echo.</summary>
    [Fact]
    public async Task Patch_me_persists_theme()
    {
        var (client, _, _, _) = await fixture.NewWorkspaceClientAsync();

        var patchResponse = await client.PatchAsJsonAsync(
            new Uri("/api/v1/users/me", UriKind.Relative),
            new { displayName = "Renamed Person", theme = "dark" });
        patchResponse.StatusCode.ShouldBe(HttpStatusCode.OK);
        var patched = await patchResponse.Content.ReadFromJsonAsync<CurrentUserResp>();
        patched!.Theme.ShouldBe("dark");

        var getResponse = await client.GetFromJsonAsync<CurrentUserResp>("/api/v1/users/me");
        getResponse!.Theme.ShouldBe("dark");
    }

    [Fact]
    public async Task Patch_me_rejects_invalid_theme()
    {
        var (client, _, _, _) = await fixture.NewWorkspaceClientAsync();

        var response = await client.PatchAsJsonAsync(
            new Uri("/api/v1/users/me", UriKind.Relative),
            new { displayName = "Renamed Person", theme = "solarized" });

        response.StatusCode.ShouldBe(HttpStatusCode.BadRequest);
    }

    [Fact]
    public async Task Patch_me_rejects_invalid_timezone()
    {
        var (client, _, _, _) = await fixture.NewWorkspaceClientAsync();

        var response = await client.PatchAsJsonAsync(
            new Uri("/api/v1/users/me", UriKind.Relative),
            new { displayName = "Renamed Person", timezone = "Not/AZone" });

        response.StatusCode.ShouldBe(HttpStatusCode.BadRequest);
    }

    [Fact]
    public async Task Patch_me_rejects_blank_display_name()
    {
        var (client, _, _, _) = await fixture.NewWorkspaceClientAsync();

        var response = await client.PatchAsJsonAsync(
            new Uri("/api/v1/users/me", UriKind.Relative), new { displayName = "   " });
