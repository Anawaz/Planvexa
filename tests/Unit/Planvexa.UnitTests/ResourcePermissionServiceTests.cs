...
public sealed class ResourcePermissionServiceTests
{
    ...
    private static ResourcePermissionService BuildService(
        FakeResourcePermissionStore acl, FakeHierarchyQuery hierarchy, WorkspaceMember? member,
        IReadOnlyList<Guid>? teamIds = null, IAuditWriter? audit = null)
    {
    ...
    [Fact]
    public async Task GrantAsync_regrant_audits_both_the_previous_and_new_level()
    {
        var acl = new FakeResourcePermissionStore();
        var taskId = Guid.NewGuid();
        acl.Add("task", taskId, ResourcePrincipalType.User, UserId, ResourcePermissionLevel.View);

        var hierarchy = new FakeHierarchyQuery();
        var audit = new CapturingAuditWriter();
        var svc = BuildService(acl, hierarchy, member: null, audit: audit);

        await svc.GrantAsync(WorkspaceId, UserId, "task", taskId, "user", UserId, PermissionLevel.Edit);

        var entry = audit.Entries.Single(e => e.Action == "resource_permission.updated");
        entry.GetString("previousLevel").ShouldBe("View");
        entry.GetString("newLevel").ShouldBe("Edit");
    }
    ...
    private sealed class FakeResourcePermissionStore : IResourcePermissionStore
    {
        ...
        public Task<IReadOnlySet<Guid>> ListResourceIdsWithGrantsAsync(
            ...
            => Task.FromResult<IReadOnlySet<Guid>>(_grants
                .Where(g => g.ResourceType == resourceType && resourceIds.Contains(g.ResourceId))
