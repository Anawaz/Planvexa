...
public sealed class PersonalAccessTokenTests
{
    ...
    public void Create_returns_prefixed_raw_and_stores_only_hash()
    {
        var (token, raw) = PersonalAccessToken.Create(
            ...
            "ci", new[] { PatScopes.TasksRead, PatScopes.TasksWrite }, null, DateTimeOffset.UtcNow);

        raw.ShouldStartWith("pat_");
        token.TokenHash.ShouldBe(SecretCrypto.Hash(raw));
        token.TokenHash.ShouldNotContain(raw);
        token.Scopes.ShouldBe(new[] { PatScopes.TasksRead, PatScopes.TasksWrite });
    }

    [Fact]
    public void Create_drops_unrecognized_scope_strings_leaving_a_legacy_full_access_token()
    {
        // Unknown scopes (e.g. from a token minted before the scope vocabulary existed, or a typo) are
        // silently dropped rather than rejected — a token left with zero recognized scopes is treated as
        // full-access for backward compatibility (see OAuthScopeEnforcementMiddleware).
        var (token, _) = PersonalAccessToken.Create(
            Guid.CreateVersion7(), Guid.CreateVersion7(), Guid.CreateVersion7(), "sub", "e@x.io", "Ed",
            "ci", new[] { "read", "not-a-real-scope" }, null, DateTimeOffset.UtcNow);

        token.Scopes.ShouldBeEmpty();
    }
