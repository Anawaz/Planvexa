...
public sealed class VapidKeyProvider
{
    ...
    /// <summary>The application server's ECDSA P-256 keypair (private key included). Exposed so
    /// <see cref="WebPushSender"/> can sign the RFC 8292 VAPID JWT with <c>ECDsa.SignData</c> -- never
    /// serialize this out of the process.</summary>
    public ECDsa Key => _key;

    /// <summary>Uncompressed public key point (0x04 || X || Y, 65 bytes).</summary>
    ...
    /// format the browser's <c>PushManager.subscribe({ applicationServerKey })</c> expects, and the same
    /// format the <c>k=</c> VAPID Authorization parameter (RFC 8292) uses.</summary>
    public string PublicKeyBase64Url
        => Convert.ToBase64String(PublicKeyBytes).Replace('+', '-').Replace('/', '_').TrimEnd('=');
}
