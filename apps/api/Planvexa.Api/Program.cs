...
// ---- Malware scanning: real clamd (ClamdMalwareScanner) when ClamAv:Host is configured, mirroring
// SmtpEmailSender's config-presence gate (not an environment-name gate, so a clamd sidecar wired up in
// any environment -- including Production -- is used, and CI/local dev without one falls back safely). ----
if (!string.IsNullOrWhiteSpace(builder.Configuration["ClamAv:Host"]))
{
    builder.Services.AddSingleton<Planvexa.BuildingBlocks.Abstractions.IMalwareScanner, Planvexa.Api.Storage.ClamdMalwareScanner>();
}
else
{
...
// Push delivery: real Web Push (RFC 8291/8292, see WebPushSender's doc comment) in Development, mirroring
// SmtpEmailSender's gate. Testing (and any other non-Development environment) keeps LoggingPushSender so
// integration tests assert delivery via SentPushLog instead of POSTing to a real browser push endpoint.
// Native iOS/Android (FCM/APNs) is still a documented gap WebPushSender does not close.
builder.Services.AddSingleton<SentPushLog>();
if (builder.Environment.IsDevelopment())
{
    builder.Services.AddHttpClient(WebPushSender.ClientName, client => client.Timeout = TimeSpan.FromSeconds(10));
    builder.Services.AddScoped<Planvexa.Modules.Notifications.Application.IPushSender, WebPushSender>();
}
else
{
