...
/// </summary>
public sealed class WorkspaceQaService(
    SearchAggregator aggregator, AiServiceContext aiCtx, IAiRequestStore requests, IAiCompletionProvider provider,
    IAiProviderSettingsStore settingsStore)
{
    ...
    public async Task<AiAskDto> AskAsync(string? question, CancellationToken ct)
    {
        ...
        var aiFeaturesEnabled = (await settingsStore.FindAsync(workspace.WorkspaceId, ct))?.AiFeaturesEnabled ?? true;
        AiAuthorizer.EnsureUse(role, aiFeaturesEnabled);
        ...
        await AiCreditLimitGuard.EnsureWithinLimitAsync(settingsStore, requests, workspace.WorkspaceId, aiCtx.Clock.UtcNow, ct);
