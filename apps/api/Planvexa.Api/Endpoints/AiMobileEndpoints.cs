...
/// <summary>A null/blank <c>ApiKey</c> keeps the stored key (write-only secret). <c>CreditLimit</c> is a
/// monthly cap (calendar month, UTC) on estimated tokens spent through the real provider; null/omitted means unlimited.</summary>
public sealed record UpdateAiProviderSettingsRequest(string? BaseUrl, string? Model, string? ApiKey, bool IsEnabled, int? CreditLimit = null);
...
public sealed record UpdateAiFeaturesEnabledRequest(bool Enabled);
...
public static class AiMobileEndpoints
{
    ...
    private static void MapAi(RouteGroupBuilder api)
    {
        ...
        // Member+-readable master switch (distinct from the Admin+-only /settings) so clients can decide
        // whether to show AI entry points at all without needing settings-management access.
        group.MapGet("/features", async (AiSettingsService svc, CancellationToken ct) =>
            Results.Ok(await svc.GetFeatureStatusAsync(ct)));
        ...
        // The "allow AI to be completely disabled" master switch (Admin+), distinct from IsEnabled above.
        group.MapPut("/settings/features", async (UpdateAiFeaturesEnabledRequest r, AiSettingsService svc, CancellationToken ct) =>
            Results.Ok(await svc.UpdateFeaturesEnabledAsync(r.Enabled, ct)));
    ...
    private static UpdateAiProviderSettingsCommand ToCommand(UpdateAiProviderSettingsRequest? r)
        => new(r?.BaseUrl ?? string.Empty, r?.Model ?? string.Empty, r?.ApiKey, r?.IsEnabled ?? false, r?.CreditLimit);
