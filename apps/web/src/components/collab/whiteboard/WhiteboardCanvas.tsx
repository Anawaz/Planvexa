...
export function WhiteboardCanvas({
  ...
  function confirmLink() {
    if (!linkPicker || !linkPicker.resourceId) return;
    const id = newShapeId();
    upsert({
      id,
      type: "link",
      ...
      width: 180,
      height: 60,
      resourceType: linkPicker.resourceType,
      resourceId: linkPicker.resourceId,
      label: linkPicker.label,
    });
    setSelectedId(id);
    setTool("select");
    setLinkPicker(null);
    ...
    <div className="space-y-2">
      <div className="flex flex-wrap items-center gap-1.5 rounded-[var(--radius)] border border-border bg-card p-2">
        ...
        <Button
          ...
          variant="outline"
          disabled={!canEdit}
          onClick={() => setLinkPicker({ resourceType: "task", resourceId: "", label: "" })}
        >
          Link task/doc
        </Button>
