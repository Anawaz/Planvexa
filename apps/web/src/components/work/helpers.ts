...
/**
 * `locale`/`timeZone` are optional overrides; unset they default to the signed-in user's saved
 * preference (AppContext -> setFormatPreferences), then to the browser's ambient locale/timezone —
 * same fallback chain as planning/helpers.ts's date formatters.
 */
