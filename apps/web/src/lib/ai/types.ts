...
export type AiProviderSettings = {
  ...
  apiKeyMask: string;
  isEnabled: boolean;
  /** Master switch: when false, every AI action is rejected for this workspace regardless of `isEnabled`. */
  aiFeaturesEnabled: boolean;
  /** Monthly cap (calendar month, UTC) on estimated tokens spent through the real provider. Null means unlimited. */
  creditLimit?: number | null;
};
...
export type UpdateAiProviderSettingsInput = {
  ...
  apiKey?: string;
  isEnabled: boolean;
  /** Monthly cap (calendar month, UTC) on estimated tokens spent through the real provider. Null/omitted means unlimited. */
  creditLimit?: number | null;
};

/** The master switch alone, readable by any workspace Member+ (not just Admin+). */
