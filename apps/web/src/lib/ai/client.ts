...
/** Member+-readable master switch — use this (not `getAiProviderSettings`, which is Admin+-only) to
 * decide whether to show AI entry points. */
export function getAiFeatureStatus() {
  return apiClient.get<AiFeatureStatus>("/ai/features");
}
...
export function updateAiFeaturesEnabled(enabled: boolean) {
  return apiClient.put<AiProviderSettings>("/ai/settings/features", { enabled });
}
