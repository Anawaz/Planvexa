...
export function useMembers() {
  const { workspaceId, currentWorkspace } = useAppContext();

  return useQuery({
    queryKey: ["members", workspaceId],
    queryFn: () => apiClient.get<Member[]>(`/workspaces/${workspaceId}/members`),
    // Guests lack TenancyPermissions.MembersView server-side (403) — skip the request rather than
    // let it fail noisily; useMemberDirectory already falls back to raw user ids without it.
    enabled: Boolean(workspaceId) && currentWorkspace?.role !== "Guest",
  });
}
...
export function useTeams(enabled = true) {
  ...
  return useQuery({
    ...
    // Guests lack workspace-team read access server-side (403) — see useMembers above.
    enabled: enabled && Boolean(workspaceId) && currentWorkspace?.role !== "Guest",
  });
}
...
export function useMemberDirectory() {
  ...
  return useMemo(() => {
    ...
    return {
      ...
      getAvatarUrl: (userId: string) => byUserId.get(userId)?.avatarUrl ?? null,
    };
  }, [data]);
}
