...
/**
 * Maps a backend entityType to the query roots it invalidates. The API emits exactly these six
 * (grep `new RealtimeEvent(`): Task, Comment, ChatChannel, ChatMessage, TimeEntry, Notification.
 * Unknown types are ignored rather than triggering a blanket invalidate — a future emitter gets an
 * entry here.
 */
export function invalidateFor(queryClient: QueryClient, event: EntityChangedEvent, workspaceId: string, currentUserId: string | undefined) {
  ...
  switch (event.entityType) {
    ...
    case "TimeEntry":
      ...
      break;
    case "Notification":
      // entityId carries the recipient (NotificationPublisher), not a notification id — every
      // connection in the workspace group gets this event, so only react to our own.
      if (event.entityId === currentUserId) {
        invalidate(collabKeys.unreadCount());
        invalidate(collabKeys.notificationsRoot());
      }
      break;
...
export function useRealtime() {
  ...
  const { workspaceId, mfaRequired, currentUserId } = useAppContext();

  useEffect(() => {
    // WorkspaceHub also rejects JoinWorkspace server-side when MFA is required and unmet; skip opening
    // the connection at all rather than opening it just to have it thrown out.
    if (!workspaceId || mfaRequired) return;
    ...
    connection.on("entityChanged", (event: EntityChangedEvent) => {
      ...
      invalidateFor(queryClient, event, workspaceId, currentUserId);
    });
    ...
    connection.onreconnected(() => {
      // A reconnect can land after this effect already tore down (e.g. logout clearing
      // workspaceId) — the closure's workspaceId is stale at that point, so skip rejoining.
      if (disposed) return;
  ...
  }, [workspaceId, queryClient, mfaRequired, currentUserId]);
}
