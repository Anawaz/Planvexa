...
export const NOTIFICATION_EVENT_LABELS: Record<string, string> = {
  ...
  status_change: "Task status changes",
...
/** A public, view-only document share link (DocumentShareLinkDto). Unlike task ShareLink, there is no
 * permission level or guest-comment support — documents-public-sharing is view-only. */
...
/** Anonymous projection returned by GET /public/documents/{token} — the document's title and its
 * content rendered as Markdown (never the raw Lexical JSON). */
...
export type AddCommentInput = {
  ...
  parentId?: string | null;
  mentionUserIds?: string[];
  /** Uploaded separately, right after the comment is created (see CommentThread's addMutation) —
   * never sent as part of the JSON comment-creation request itself. */
  file?: File | null;
};
...
// ADR-0003 private sharing (mirrors SharedContracts.Workspaces.PermissionLevel/ResourcePrincipalType,
// same vocabulary as WorkManagement's resource_permissions — see lib/work/sharing.ts). Documents only
// ever grants User/Team principals a View or Edit level (see DocumentsAuthorizer.CanModifyPrivateAsync).
export type DocumentPrincipalType = "user" | "team";
