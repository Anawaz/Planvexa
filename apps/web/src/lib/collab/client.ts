...
/** Uploads a file attached to an already-created comment — same multipart-body convention as
 * uploadAttachment (WorkManagement's task attachments). */
export function uploadCommentAttachment(commentId: string, file: File) {
  const body = new FormData();
  body.append("file", file);
  return apiClient.post<CommentAttachment, FormData>(`/comments/${commentId}/attachments`, body);
}

export function deleteCommentAttachment(id: string) {
  return apiClient.delete<void>(`/comment-attachments/${id}`);
}

/** Plain `<a href>` download target — the proxy re-applies the workspace header (mirrors attachmentDownloadHref). */
export function commentAttachmentDownloadHref(id: string) {
  return proxyHref(`/comment-attachments/${id}/download`);
}
...
export function addDocumentComment(documentId: string, body: string) {
  return apiClient.post<DocumentComment>(`/documents/${documentId}/comments`, { body });
}

/** Uploads an image dropped/pasted/inserted into a document's rich-text content (see the editor's
 * ImageNode) — same multipart-body convention as uploadWhiteboardImage. */
export function uploadDocumentImage(documentId: string, file: File | Blob) {
  const body = new FormData();
  body.append("file", file);
  return apiClient.post<{ imageId: string; contentType: string }, FormData>(`/documents/${documentId}/images`, body);
}

/** Plain `<img>` src — the proxy re-applies the workspace header (mirrors whiteboardImageHref). */
export function documentImageHref(documentId: string, imageId: string) {
  return proxyHref(`/documents/${documentId}/images/${imageId}`);
}

/** Uploads a file attached (not rendered inline) to a document's rich-text content (see the editor's
 * FileAttachmentNode) — same multipart-body convention as uploadDocumentImage. */
export function uploadDocumentAttachment(documentId: string, file: File) {
  const body = new FormData();
  body.append("file", file);
  return apiClient.post<{ attachmentId: string; fileName: string; contentType: string; sizeBytes: number }, FormData>(
    `/documents/${documentId}/attachments`,
    body,
  );
}

/** Plain `<a href>` download link — the server responds with Content-Disposition: attachment. */
export function documentAttachmentHref(documentId: string, attachmentId: string, fileName: string) {
  return proxyHref(`/documents/${documentId}/attachments/${attachmentId}/${encodeURIComponent(fileName)}`);
}
...
// ---- document public share links ----
// View-only — no permission level or guest comments, unlike task share links.

export function createDocumentShare(documentId: string, expiresInDays?: number, password?: string) {
  return apiClient.post<DocumentShareLink>(`/documents/${documentId}/share`, {
    expiresInDays: expiresInDays ?? null,
    password: password || null,
  });
}
...
export function listDocumentShares(documentId: string) {
  return apiClient.get<DocumentShareLink[]>(`/documents/${documentId}/shares`);
}

export function revokeDocumentShare(id: string) {
  return apiClient.delete<void>(`/document-shares/${id}`);
}

// ---- private sharing with specific Users/Teams (ADR-0003) ----
// A private document is otherwise visible only to its owner — these grant explicit View/Edit access
// to a chosen User or Team, resolved via ResourcePicker rather than a raw id field.

export function listDocumentPermissions(documentId: string) {
  return apiClient.get<ResourcePermissionGrant[]>(`/documents/${documentId}/permissions`);
}
...
export function grantDocumentPermission(
  ...
  return apiClient.post<ResourcePermissionGrant>(`/documents/${documentId}/permissions`, {
    principalType,
    principalId,
    level,
  });
...
export function revokeDocumentPermission(documentId: string, principalType: DocumentPrincipalType, principalId: string) {
  return apiClient.delete<void>(`/documents/${documentId}/permissions/${principalType}/${principalId}`);
}

/** Anonymous: read straight from the API origin (the BFF proxy requires a session cookie), same as
 * getPublicSharedTask. No password prompt UI, same gap as the task public page today — a
 * password-protected link 404s here rather than prompting (see PublicCommentForm's doc comment for the
 * same limitation on the comment-posting side). */
export function getPublicSharedDocument(token: string) {
  return getPublic<SharedDocument>(`/public/documents/${encodeURIComponent(token)}`);
}
...
export function retryWebhookDelivery({ webhookId, deliveryId }: { webhookId: string; deliveryId: string }) {
  return apiClient.post<WebhookDelivery>(`/webhooks/${webhookId}/deliveries/${deliveryId}/retry`);
}
