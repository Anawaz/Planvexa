...
/** My Work personal sort/organize preferences (product spec section 15) — global to the user, not
 * scoped to any one Workspace, since My Work itself can span every Workspace the user belongs to. */
export type MyWorkSortBy = "dueDate" | "priority" | "title";
export type MyWorkSection = "created" | "watching";
export type MyWorkPreferences = {
  sortBy: MyWorkSortBy;
  hiddenSections: MyWorkSection[];
};
...
export type StatusDefinition = {
  ...
  color: string;
  position: number;
  /** Statuses this one may transition to. Empty means unrestricted (any status in the scheme) —
   * enforced by the backend, this is not merely a UI hint. */
...
export type Task = {
  ...
  isArchived: boolean;
  /**  . uc(t)he user who created this task, if known (system-generated tasks may have none). */
