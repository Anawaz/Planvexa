...
internal sealed class PushDeviceDirectory(PlanvexaDbContext db) : IPushDeviceDirectory
{
    ...
    public async Task<IReadOnlyList<PushSubscription>> ListSubscriptionsAsync(Guid workspaceId, Guid userId, CancellationToken cancellationToken = default)
        => await db.Set<DeviceRegistration>()
            .Where(x => x.WorkspaceId == workspaceId && x.UserId == userId
                && x.PushEndpoint != null && x.PushP256dh != null && x.PushAuth != null)
            .Select(x => new PushSubscription(x.Id, x.PushEndpoint!, x.PushP256dh!, x.PushAuth!))
            .ToListAsync(cancellationToken);

    public async Task MarkPushSubscriptionExpiredAsync(Guid deviceId, CancellationToken cancellationToken = default)
    {
        var device = await db.Set<DeviceRegistration>().FirstOrDefaultAsync(x => x.Id == deviceId, cancellationToken);
        device?.ClearPushSubscription();
    }
}
