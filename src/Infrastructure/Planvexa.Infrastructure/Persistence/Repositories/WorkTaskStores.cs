...
internal sealed class WorkItemStore(PlanvexaDbContext db) : IWorkItemStore
{
    ...
    public void Remove(WorkItem task) => db.Set<WorkItem>().Remove(task);
    ...
    public async Task<IReadOnlyList<WorkItem>> ListAssignedToUserAsync(Guid userId, Guid? workspaceId = null, CancellationToken ct = default)
    {
        ...
        var query = db.Set<WorkItem>().Include(x => x.Assignees).Include(x => x.Tags)
    ...
    public async Task<IReadOnlyList<WorkItem>> ListCreatedByUserAsync(Guid userId, Guid? workspaceId = null, CancellationToken ct = default)
    {
        var query = db.Set<WorkItem>().Include(x => x.Assignees).Include(x => x.Tags)
    ...
    public async Task<IReadOnlyList<WorkItem>> ListWatchedByUserAsync(Guid userId, Guid? workspaceId = null, CancellationToken ct = default)
    {
        var taskIds = db.Set<TaskWatcher>()
