    ...
    : IWorkReportingQueries
{
    ...
    public async Task<IReadOnlyList<Guid>> UnassignedTaskIdsAsync(Guid workspaceId, CancellationToken ct = default)
        => await db.Set<WorkItem>()
            .Where(t => t.WorkspaceId == workspaceId && !t.IsDeleted && !t.IsCompleted && !t.Assignees.Any())
            .Select(t => t.Id)
            .ToListAsync(ct);
    ...
    public async Task<IReadOnlyList<PortfolioSpaceRow>> PortfolioAsync(Guid workspaceId, IReadOnlyCollection<Guid> spaceIds, CancellationToken ct = default)
    {
        if (spaceIds.Count == 0)
        {
            return Array.Empty<PortfolioSpaceRow>();
        }

        var rows = await (
            from t in db.Set<WorkItem>()
            join sp in db.Set<Space>() on t.SpaceId equals sp.Id
            where t.WorkspaceId == workspaceId && !t.IsDeleted && spaceIds.Contains(sp.Id)
            group new { t.IsCompleted } by new { sp.Id, sp.Name } into g
            select new
            {
                g.Key.Id,
    ...
    public async Task<IReadOnlyDictionary<Guid, int>> TaskCountsByAssigneeAsync(Guid workspaceId, CancellationToken ct = default)
    {
        var rows = await (
            from a in db.Set<TaskAssignee>()
            join t in db.Set<WorkItem>() on a.TaskId equals t.Id
    ...
    public async Task<IReadOnlyList<PriorityCount>> PriorityCountsAsync(Guid workspaceId, CancellationToken ct = default)
    {
        var rows = await db.Set<WorkItem>()
            .Where(t => t.WorkspaceId == workspaceId && !t.IsDeleted)
        ...
        return rows.Select(r => new PriorityCount(r.Priority.ToString(), r.Count)).ToList();
    }

    public async Task<int> CreatedCountAsync(Guid workspaceId, DateTimeOffset fromUtc, DateTimeOffset toUtc, CancellationToken ct = default)
        => await db.Set<WorkItem>()
            .CountAsync(t => t.WorkspaceId == workspaceId && !t.IsDeleted
                && t.CreatedAtUtc >= fromUtc && t.CreatedAtUtc < toUtc, ct);

    /// <summary>
    /// ponytail: groups by whichever single typed column is populated (Dropdown -> option label,
    /// User/Team -> raw id, everything else -> its stored value); MultiSelect isn't single-valued so a
    /// task in 2 options would need to count in both buckets — skipped here, add a JsonValue fan-out
    /// pass if MultiSelect breakdowns are needed. Values are already workspace-scoped via
    /// CustomFieldValue.WorkspaceId, so a definitionId from another workspace simply yields no rows.
    /// </summary>
    public async Task<IReadOnlyList<CustomFieldValueCount>> CustomFieldValueCountsAsync(Guid workspaceId, Guid definitionId, CancellationToken ct = default)
    {
        var values = await db.Set<CustomFieldValue>()
            .Where(v => v.WorkspaceId == workspaceId && v.DefinitionId == definitionId)
            .Select(v => new { v.TextValue, v.NumberValue, v.DateValue, v.BoolValue, v.OptionId, v.UserValue, v.TeamValue })
            .ToListAsync(ct);
        if (values.Count == 0)
        {
            return Array.Empty<CustomFieldValueCount>();
        }

        var optionIds = values.Where(v => v.OptionId is not null).Select(v => v.OptionId!.Value).Distinct().ToList();
        var optionLabels = optionIds.Count == 0
            ? new Dictionary<Guid, string>()
            : await db.Set<CustomFieldOption>().Where(o => optionIds.Contains(o.Id)).ToDictionaryAsync(o => o.Id, o => o.Label, ct);

        string Label(Guid? optionId, string? text, decimal? number, bool? boolean, DateTimeOffset? date, Guid? user, Guid? team)
            => optionId is { } id ? optionLabels.GetValueOrDefault(id, "(Unknown option)")
                : !string.IsNullOrWhiteSpace(text) ? text
                : number is { } n ? n.ToString(System.Globalization.CultureInfo.InvariantCulture)
                : boolean is { } b ? (b ? "Yes" : "No")
                : date is { } d ? d.ToString("yyyy-MM-dd")
                : user is { } u ? u.ToString()
                : team is { } tm ? tm.ToString()
                : "(No value)";

        return values
            .GroupBy(v => Label(v.OptionId, v.TextValue, v.NumberValue, v.BoolValue, v.DateValue, v.UserValue, v.TeamValue))
            .Select(g => new CustomFieldValueCount(g.Key, g.Count()))
