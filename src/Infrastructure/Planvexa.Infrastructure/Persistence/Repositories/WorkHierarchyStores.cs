...
internal sealed class MyWorkPreferenceStore(PlanvexaDbContext db) : IMyWorkPreferenceStore
{
    public void Add(MyWorkPreference preference) => db.Set<MyWorkPreference>().Add(preference);

    public Task<MyWorkPreference?> FindAsync(Guid userId, CancellationToken ct = default)
        => db.Set<MyWorkPreference>().FirstOrDefaultAsync(x => x.UserId == userId, ct);
}
