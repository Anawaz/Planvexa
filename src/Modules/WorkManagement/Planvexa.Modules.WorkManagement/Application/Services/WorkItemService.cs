    ...
    INotificationPublisher notifications) : WorkServiceBase(ctx)
{
    public async Task<TaskDto> CreateAsync(CreateTaskCommand command, string? idempotencyKey = null, CancellationToken ct = default)
    {
        ...
        {
            if (await Ctx.Access.GetAccessAsync(list.WorkspaceId, assignee, ct) is null)
            {
                throw new ValidationAppException("An assignee must be a member of this workspace.");
            }
    ...
    public async Task<TaskDto> AddTeamAssigneeAsync(Guid taskId, Guid teamId, CancellationToken ct = default)
    {
        ...
        if (!await teamDirectory.TeamExistsAsync(task.WorkspaceId, teamId, ct))
        {
            throw new ValidationAppException("The team must belong to this workspace.");
        }
    ...
    /// <summary>
    /// ACL-filtered task listing for a List. <paramref name="afterTaskId"/>/<paramref name="limit"/> are
    /// an optional keyset page over the same stable Position order the list is always returned in
    /// (mirrors GET /activity's before/take cursor, just keyed by the previous page's last task id
    /// instead of a timestamp -- tasks have no natural monotonic timestamp to page on, and Position can
    /// repeat after drag-drop reordering, so resuming after a specific id avoids skipping/duplicating
    /// ties). Omitting both keeps the original backward-compatible "return everything" behavior.
    /// </summary>
    public async Task<IReadOnlyList<TaskDto>> ListByListAsync(
        Guid listId, Guid? afterTaskId = null, int? limit = null, CancellationToken ct = default)
    {
        ...
        var candidates = result.Where(t => !t.IsDeleted).ToList();
        ...
        var visible = await FilterReadableInListContextAsync(candidates, list.WorkspaceId, listId, ct);

        IEnumerable<WorkItem> page = visible;
        if (afterTaskId is { } cursor)
        {
            page = page.SkipWhile(t => t.Id != cursor).Skip(1);
        }

        if (limit is { } take)
        {
            page = page.Take(Math.Clamp(take, 1, 500));
        }

        return page.Select(WorkMapper.ToDto).ToList();
    }
    ...
    {
        var visible = await ListByListAsync(listId, ct: ct);
    ...
    /// <summary>My Work "Assigned to me" section. When <paramref name="workspaceId"/> is given, scopes
    /// the result to that Workspace only (My Work's workspace filter); otherwise spans every Workspace
    /// the user belongs to.</summary>
    public async Task<IReadOnlyList<TaskDto>> ListMineAsync(Guid? workspaceId = null, CancellationToken ct = default)
    {
        var result = await tasks.ListAssignedToUserAsync(UserId, workspaceId, ct);
        return result.Where(t => !t.IsDeleted).OrderBy(t => t.DueDate ?? DateTimeOffset.MaxValue).Select(WorkMapper.ToDto).ToList();
    }

    /// <summary>My Work "Created by me" section (product spec section 15) — tasks this user created,
    /// independent of current assignment. When <paramref name="workspaceId"/> is given, scopes the
    /// result to that Workspace only.</summary>
    public async Task<IReadOnlyList<TaskDto>> ListCreatedByMeAsync(Guid? workspaceId = null, CancellationToken ct = default)
    {
        var result = await tasks.ListCreatedByUserAsync(UserId, workspaceId, ct);
        return result.Where(t => !t.IsDeleted).OrderByDescending(t => t.CreatedAtUtc).Select(WorkMapper.ToDto).ToList();
    }

    /// <summary>My Work "Watching" section (product spec section 15) — every task this user watches,
    /// independent of assignment or authorship. When <paramref name="workspaceId"/> is given, scopes
    /// the result to that Workspace only.</summary>
    public async Task<IReadOnlyList<TaskDto>> ListWatchingAsync(Guid? workspaceId = null, CancellationToken ct = default)
    {
        var result = await tasks.ListWatchedByUserAsync(UserId, workspaceId, ct);
        return result.Where(t => !t.IsDeleted).OrderBy(t => t.DueDate ?? DateTimeOffset.MaxValue).Select(WorkMapper.ToDto).ToList();
    }
    ...
    public async Task<TaskDto> UpdateAsync(Guid taskId, UpdateTaskCommand command, CancellationToken ct = default)
    {
        ...
        if (command.StatusId is { } statusId && statusId != task.StatusId)
        {
            ...
            var status = await RequireStatusInSchemeAsync(list!.StatusSchemeId, statusId, ct, task.StatusId);
            ...
            await NotifyStatusChangeAsync(task, oldStatusId, status, ct);
        }

        // Snapshot before UpdateDetails/SetTaskType/SetCustomId mutate the task, so each field's activity
        // event is only written when the requested value actually changes it (spec section 18: Dates
        // changed / Priority changed are distinct activity events, not folded into the generic "updated").
        var oldPriority = task.Priority;
        var oldStartDate = task.StartDate;
        var oldDueDate = task.DueDate;
        var oldTaskTypeId = task.TaskTypeId;
        var oldCustomId = task.CustomId;
        ...
        if (command.Priority is { } newPriority && newPriority != oldPriority)
        {
            activity.Add(new TaskActivityEvent(NewId(), task.WorkspaceId, task.Id, UserId, "priority_changed", newPriority.ToString(), Now));
        }

        if ((command.StartDate.HasValue && task.StartDate != oldStartDate) || (command.DueDate.HasValue && task.DueDate != oldDueDate))
        {
            activity.Add(new TaskActivityEvent(NewId(), task.WorkspaceId, task.Id, UserId, "dates_changed", null, Now));
        }

        if (command.TaskTypeId is not null)
        {
            task.SetTaskType(command.TaskTypeId, UserId, Now);
            if (task.TaskTypeId != oldTaskTypeId)
            {
                activity.Add(new TaskActivityEvent(NewId(), task.WorkspaceId, task.Id, UserId, "task_type_changed", null, Now));
            }
        }
        ...
        if (command.CustomId is not null)
        {
            ...
            task.SetCustomId(command.CustomId, UserId, Now);
            if (task.CustomId != oldCustomId)
            {
                activity.Add(new TaskActivityEvent(NewId(), task.WorkspaceId, task.Id, UserId, "custom_id_changed", task.CustomId, Now));
            }
        }
    ...
    public async Task<TaskDto> MoveAsync(Guid taskId, MoveTaskCommand command, CancellationToken ct = default)
    {
        ...
        if (targetList.WorkspaceId != task.WorkspaceId)
        {
            throw new ValidationAppException("Cannot move a task to a list in a different workspace.");
        }
        ...
        {
            var status = await RequireStatusInSchemeAsync(targetList.StatusSchemeId, statusId, ct, task.StatusId);
            if (status.IsCompletedCategory)
            {
                await GuardCompletionAsync(task, ct);
            }
    ...
    public async Task<TaskDto> ArchiveAsync(Guid taskId, bool archive, CancellationToken ct = default)
    {
        var (task, _) = await LoadForEditAsync(taskId, ct);
        if (archive)
        {
        ...
        Audit(archive ? "task.archived" : "task.unarchived", "Task", task.Id);
        await SaveAsync(ct);
        return WorkMapper.ToDto(task);
    }
    ...
    /// <summary>
    /// Irreversibly removes a task that is already in the trash (spec section 10.6: "Permanently
    /// delete", distinct from the recoverable soft-delete <see cref="DeleteAsync"/> performs). Requires
    /// manage-structure — a stronger bar than the edit-content a soft delete needs — since this cannot
    /// be undone. Same-schema child rows (assignees, watchers, tags, team assignees, memberships,
    /// relations, attachments, reminders, relationship custom-field values) cascade at the database
    /// level, the same DB-level cascade the scheduled retention purge
    /// (RetentionPurger/IRetentionPurger) already relies on for exactly this entity.
    /// </summary>
    public async Task PermanentDeleteAsync(Guid taskId, CancellationToken ct = default)
    {
        var task = await tasks.FindAsync(taskId, ct) ?? throw new NotFoundException("Task not found.");
        if (!task.IsDeleted)
        {
            throw new ValidationAppException("Only a task already in the trash can be permanently deleted.");
        }

        await EnsureManageStructureAsync(task, WorkResourceTypes.Task, ct);

        Audit("task.permanently_deleted", "Task", task.Id, new { task.Title });
        tasks.Remove(task);
        await SaveAsync(ct);
    }
    ...
    {
        var (task, _) = await LoadForEditWithRelationsAsync(taskId, ct);
        if (await Ctx.Access.GetAccessAsync(task.WorkspaceId, assigneeUserId, ct) is null)
        {
            throw new ValidationAppException("An assignee must be a member of this workspace.");
        }
        ...
        if (task.AddAssignee(NewId(), assigneeUserId, UserId, Now))
        {
            ...
            await NotifyAssigneeAsync(task, assigneeUserId, ct);
            await SaveAsync(ct);
        }
    ...
    /// <summary>Fires the "assignment" notification for a newly assigned user (skipped for self-assignment).
    /// Deduplicated per task+assignee by <see cref="INotificationPublisher"/> itself.</summary>
    private Task NotifyAssigneeAsync(WorkItem task, Guid assigneeUserId, CancellationToken ct)
    {
        ...
        return notifications.PublishAsync(new NotificationRequest(
            RecipientUserId: assigneeUserId,
            EventType: "assignment",
            EntityType: "Task",
            EntityId: task.Id,
            WorkspaceId: task.WorkspaceId,
            DeduplicationKey: $"assignment:{task.Id:N}:{assigneeUserId:N}",
            Payload: new Dictionary<string, string> { ["taskTitle"] = task.Title, ["byUserId"] = UserId.ToString() }), ct);
    }
    ...
    {
        var withRelations = await tasks.FindWithRelationsAsync(task.Id, ct) ?? task;
        var recipients = withRelations.Assignees.Select(a => a.UserId)
            .Concat(withRelations.Watchers.Select(w => w.UserId))
            .Distinct()
    ...
    public async Task<int> BulkUpdateAsync(BulkTaskUpdate command, CancellationToken ct = default)
    {
        ...
        foreach (var task in items.Where(t => !t.IsDeleted))
        {
            ...
            if (command.AddAssigneeUserId is { } assignee && task.AddAssignee(NewId(), assignee, UserId, Now))
            {
                await NotifyAssigneeAsync(task, assignee, ct);
            }
    ...
    /// <summary>
    /// Validates the target status belongs to this scheme and, when <paramref name="fromStatusId"/> is
    /// given (an existing task changing status, not a brand-new task's initial status), that the scheme's
    /// configured allowed-transitions graph permits this move (spec section 11 — enforced here, in the
    /// backend, not only the UI). A <paramref name="fromStatusId"/> from a DIFFERENT scheme (a cross-list
    /// move into a list with its own workflow) is not a status of THIS scheme, so
    /// <see cref="StatusScheme.CanTransition"/> correctly treats it as unrestricted rather than rejecting
    /// a legitimate move.
    /// </summary>
    private async Task<StatusDefinition> RequireStatusInSchemeAsync(Guid schemeId, Guid statusId, CancellationToken ct, Guid? fromStatusId = null)
    {
        var status = await schemes.FindStatusAsync(statusId, ct);
        ...
        if (fromStatusId is { } fromId && fromId != statusId)
        {
            var scheme = await schemes.FindAsync(schemeId, ct)
                ?? throw new NotFoundException("The list's status scheme is missing.");
            if (!scheme.CanTransition(fromId, statusId))
            {
                throw new ValidationAppException($"Cannot move a task from '{scheme.Statuses.FirstOrDefault(s => s.Id == fromId)?.Name}' to '{status.Name}' — this transition is not allowed by the workflow.");
            }
        }
