    ...
    : WorkServiceBase(ctx)
{
    ...
    {
        var (attachment, task) = await LoadWithTaskAsync(id, ct);
        await EnsureReadAsync(task, WorkResourceTypes.Task, ct);
    ...
    /// <summary>Loads the attachment together with its owning task, so callers can run the same
    /// per-task privacy/ACL check (<see cref="WorkServiceBase.EnsureReadAsync"/>/EnsureEditContentAsync)
    /// a normal task read uses — the coarse workspace-role check this used to stop at let any workspace
    /// member read/delete an attachment on a private task they have no grant on.</summary>
    private async Task<(TaskAttachment Attachment, WorkItem Task)> LoadWithTaskAsync(Guid id, CancellationToken ct)
    {
        var attachment = await attachments.FindAsync(id, ct) ?? throw new NotFoundException("Attachment not found.");
        var task = await tasks.FindAsync(attachment.TaskId, ct) ?? throw new NotFoundException("Attachment not found.");
