...
public sealed class CustomFieldService(
    ...
    ITaskListStore lists, IFolderStore folders, ITeamDirectoryQuery teamDirectory) : WorkServiceBase(ctx)
{
    ...
    public async Task<CustomFieldValueDto> SetValueAsync(Guid taskId, Guid definitionId, string? rawValue, CancellationToken ct = default)
    {
        ...
        Activity(task.WorkspaceId, task.Id, "custom_field_changed", definition.Name);
    ...
    /// <summary>
    /// User and Team are the two types that need an async cross-module workspace-ownership check, so
    /// they stay here; every other settable type's parsing/validation is shared with
    /// <c>TaskWriteApi.SetCustomFieldValueAsync</c> (Forms' custom-field mapping) via
    /// <see cref="CustomFieldValueCoercion"/> — see that class's doc comment.
    /// </summary>
    private async Task ApplyTypedValueAsync(Guid workspaceId, CustomFieldDefinition definition, CustomFieldValue value, string? rawValue, CancellationToken ct)
    {
        ...
        if (definition.Type == CustomFieldType.Team)
        {
            if (!Guid.TryParse(rawValue, out var teamId) || !await teamDirectory.TeamExistsAsync(workspaceId, teamId, ct))
            {
                throw new ValidationAppException($"'{definition.Name}' must reference a team in this workspace.");
            }
        }
