...
public interface IWorkItemStore
{
    ...
    Task<IReadOnlyList<WorkItem>> ListAssignedToUserAsync(Guid userId, Guid? workspaceId = null, CancellationToken ct = default);

    /// <summary>My Work "Created by me" section: tasks whose <see cref="WorkItem.CreatedByUserId"/> is
    /// this user, regardless of current assignment. When <paramref name="workspaceId"/> is given, the
    /// result is scoped to that Workspace only; otherwise it spans every Workspace the user belongs to.</summary>
    Task<IReadOnlyList<WorkItem>> ListCreatedByUserAsync(Guid userId, Guid? workspaceId = null, CancellationToken ct = default);

    /// <summary>My Work "Watching" section: every task this user has a <see cref="TaskWatcher"/> row
    /// on, regardless of assignment or authorship. When <paramref name="workspaceId"/> is given, the
    /// result is scoped to that Workspace only; otherwise it spans every Workspace the user belongs to.</summary>
    Task<IReadOnlyList<WorkItem>> ListWatchedByUserAsync(Guid userId, Guid? workspaceId = null, CancellationToken ct = default);
...
/// <summary>Global, per-user (not per-Workspace) My Work sort/organize preferences — see
/// MyWorkPreference's doc comment for why this store is not workspace-scoped like its siblings above.</summary>
public interface IMyWorkPreferenceStore
{
    void Add(MyWorkPreference preference);
    Task<MyWorkPreference?> FindAsync(Guid userId, CancellationToken ct = default);
}
