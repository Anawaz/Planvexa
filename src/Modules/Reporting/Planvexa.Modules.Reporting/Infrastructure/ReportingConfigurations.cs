...
public sealed class PortfolioConfiguration : IEntityTypeConfiguration<Portfolio>
{
    ...
    {
        b.ToTable("portfolios", ReportingModule.Schema);
        b.HasKey(x => x.Id);
        b.Property(x => x.Name).HasMaxLength(200).IsRequired();
        b.Property(x => x.Status).HasConversion<string>().HasMaxLength(16).IsRequired();
        b.HasIndex(x => x.WorkspaceId);

        b.HasMany(x => x.Members).WithOne().HasForeignKey(m => m.PortfolioId).OnDelete(DeleteBehavior.Cascade);
        b.Navigation(x => x.Members).UsePropertyAccessMode(PropertyAccessMode.Field);
        b.Ignore(x => x.DomainEvents);
    }
}

public sealed class PortfolioMemberConfiguration : IEntityTypeConfiguration<PortfolioMember>
{
    public void Configure(EntityTypeBuilder<PortfolioMember> b)
    {
        b.ToTable("portfolio_members", ReportingModule.Schema);
        b.HasKey(x => x.Id);
        b.HasIndex(x => x.PortfolioId);
        b.HasIndex(x => new { x.PortfolioId, x.SpaceId }).IsUnique();
        b.Ignore(x => x.DomainEvents);
    }
}
