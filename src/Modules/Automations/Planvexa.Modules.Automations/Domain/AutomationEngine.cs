...
public sealed record AutomationAction(string Type, string Value)
{
    public static class Types
    {
        ...
        /// <summary>Removes a tag by name from the task (Value is the tag name — same shape as
        /// <see cref="AddTag"/>, see ITaskWriteApi.RemoveTagByNameAsync). Idempotent no-op if the task
        /// doesn't have the tag.</summary>
        public const string RemoveTag = "remove_tag";

        public const string Notify = "notify";

        /// <summary>Sets the task's priority by (case-insensitive) enum name
        /// ("Low"/"Normal"/"High"/"Urgent"/"None") — Value shape matches ITaskWriteApi.SetPriorityByNameAsync,
        /// same as Forms' TargetPriority routing.</summary>
        public const string SetPriority = "set_priority";

        /// <summary>Assigns a Team to the task (Value is the team's guid — same opaque cross-module id
        /// as Forms' TargetTeamId routing, see ITaskWriteApi.AssignTeamAsync).</summary>
        ...
        /// <summary>Calls a workspace-configured third-party integration (Value is JSON:
        /// <c>{"provider":"slack"|"github","message":"...","issueNumber":"..."}</c>, <c>message</c>
        /// interpolated with <c>{{...}}</c> tokens same as Email/Comment; <c>issueNumber</c> is required
        /// only for the "github" provider). Only "slack" (posts to the configured incoming webhook) and
        /// "github" (comments on the given issue) have a real outbound call wired up — see
        /// IntegrationProviders.RealImplementation; every other provider is settings-scaffolding only and
        /// this action reports a clear "not implemented" failure for it rather than a faked success. See
        /// AutomationDispatcher.ApplyIntegrationAsync.</summary>
        public const string Integration = "integration";

        public static readonly IReadOnlyList<string> All = new[]
        {
            SetStatus, Assign, AddTag, RemoveTag, Notify, Email, Webhook, CustomField, Comment, SetDueDateBusinessDays, Integration,
            SetPriority, AssignTeam,
        };
    }
}
