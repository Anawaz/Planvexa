...
/// through <see cref="Authorization.DocumentsAuthorizer.CanViewAsync"/> (the same private-owner-or-ACL-grant
/// check <see cref="DocumentService"/> applies to GET/list) before it is returned — see ISearchProvider's doc comment on why this filter is not
    ...
    : DocumentsServiceBase(ctx), ISearchProvider
{
    public async Task<IReadOnlyList<SearchHit>> SearchAsync(string term, int limit, CancellationToken cancellationToken = default)
    {
        ...
        var viewable = new List<Document>();
        foreach (var document in list)
        {
            if (await DocumentsAuthorizer.CanViewAsync(document, UserId, Ctx.ResourcePermissions, cancellationToken))
            {
                viewable.Add(document);
            }
        }
