...
public sealed class TimesheetPeriod : Entity, IAggregateRoot, IWorkspaceOwned
{
    ...
    /// <summary>Reopens a locked or approved period back to Draft so its entries can be edited and
    /// resubmitted (admin-only correction path; gated by the caller via <see cref="TimesheetService"/>).</summary>
    public void Reopen(DateTimeOffset nowUtc)
    {
        if (Status is not (ApprovalStatus.Locked or ApprovalStatus.Approved))
        {
            throw new ConflictException("Only a locked or approved period can be reopened.");
        }
