...
public sealed class TimeEntry : Entity, IAggregateRoot, IWorkspaceOwned
{
    ...
    /// <summary>Server instant the timer was paused at; null while running or stopped. Set/cleared only
    /// by <see cref="Pause"/>/<see cref="Resume"/>.</summary>
    public DateTimeOffset? PausedAtUtc { get; private set; }

    /// <summary>Total seconds excluded from <see cref="DurationSeconds"/> across all completed pause
    /// intervals (folded in by <see cref="Resume"/>, and by <see cref="Stop"/> for an interval still
    /// open at stop time). Server-authoritative, same as everything else here.</summary>
    ...
    /// <summary>Pauses a running timer. Time from here until <see cref="Resume"/> (or <see cref="Stop"/>)
    /// is excluded from the final duration.</summary>
    public void Pause(DateTimeOffset nowUtc)
    {
        ...
        if (IsPaused)
        {
            throw new ConflictException("This timer is already paused.");
        }
    ...
    /// <summary>Resumes a paused timer, folding the paused interval into <see cref="PausedSeconds"/>.</summary>
    public void Resume(DateTimeOffset nowUtc)
    {
        ...
        if (!IsPaused)
        {
            throw new ConflictException("This timer is not paused.");
        }

        PausedSeconds += TimeMath.DurationSeconds(PausedAtUtc!.Value, nowUtc);
        PausedAtUtc = null;
        UpdatedAtUtc = nowUtc;
    }

    /// <summary>Stops a running timer, computing the duration from server timestamps, excluding any
    /// paused time (a still-open pause interval is folded in first, as if resumed at the same instant).</summary>
    public void Stop(DateTimeOffset nowUtc, string? description)
    {
        ...
        EndedAtUtc = nowUtc;
        DurationSeconds = Math.Max(0, TimeMath.DurationSeconds(StartedAtUtc, nowUtc) - PausedSeconds);
    ...
    /// <summary>Reopens the entry back to Draft (companion to <see cref="TimesheetPeriod.Reopen"/>), making it
    /// editable again.</summary>
