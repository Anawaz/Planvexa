...
/// <summary>
/// A record of one webhook delivery attempt (idempotent per subscription + event: at most one row per
/// (SubscriptionId, EventId), matching <see cref="WebhookDeliveryConfiguration"/>'s unique index). A
/// failed delivery is not automatically retried — <see cref="ApplyRetryOutcome"/> lets an Admin+ manually
/// re-attempt it (see <c>WebhookService.RetryDeliveryAsync</c>), updating this same row's Attempt/outcome
/// in place, mirroring how <c>AutomationRun</c> tracks its own Attempts on one row rather than inserting a
/// new row per attempt.
/// </summary>
public sealed class WebhookDelivery : Entity, IWorkspaceOwned
{
    ...
    private WebhookDelivery(
        ...
        int attempt, bool success, int? statusCode, string? detail, DateTimeOffset occurredAtUtc, string? payloadJson)
        : base(id)
    {
        WorkspaceId = workspaceId;
    ...
    /// <summary>The exact signed JSON body sent (or attempted), persisted so a manual retry can replay it
    /// without the original, ephemeral <see cref="Planvexa.SharedContracts.Events.WorkspaceEvent"/> still
    /// being available. Null for deliveries recorded before this field existed — those cannot be retried.</summary>
    public string? PayloadJson { get; private set; }

    public static WebhookDelivery Record(
        Guid id, Guid workspaceId, Guid subscriptionId, Guid eventId, string eventType,
        int attempt, bool success, int? statusCode, string? detail, DateTimeOffset occurredAtUtc, string? payloadJson = null)
        => new(id, workspaceId, subscriptionId, eventId, eventType, attempt, success, statusCode,
            detail is { Length: > 500 } ? detail[..500] : detail, occurredAtUtc, payloadJson);

    /// <summary>Applies the outcome of a manual retry attempt in place: bumps <see cref="Attempt"/> and
    /// overwrites the recorded outcome, so the delivery log always reflects the most recent attempt.</summary>
    public void ApplyRetryOutcome(bool success, int? statusCode, string? detail, DateTimeOffset occurredAtUtc)
    {
        Attempt++;
        Success = success;
        StatusCode = statusCode;
