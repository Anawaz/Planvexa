    ...
    IUnitOfWork unitOfWork)
{
    public async Task<CommentDto> AddAsync(CreateCommentCommand command, string? idempotencyKey = null, CancellationToken ct = default)
    {
        ...
        // Task-level privacy/ACL check (AGENTS.md rule 7 cross-module contract) — same gate as
        // ListForTaskAsync: workspace membership alone must not let a member post into a private
        // Task's comment thread they have no grant on.
        if (!await linkedResources.CanViewAsync(task.WorkspaceId, currentUser.UserId, LinkedResourceTypes.Task, task.TaskId, ct))
        {
    ...
    public async Task<IReadOnlyList<CommentDto>> ListForTaskAsync(Guid taskId, CancellationToken ct = default)
    {
        ...
        // Task-level privacy/ACL check (AGENTS.md rule 7 cross-module contract) — workspace membership
        // alone used to let any member read comments on a private Task they have no grant on, the same
        // gap WorkManagement's own AttachmentService.LoadWithTaskAsync closed for Task attachments.
        if (!await linkedResources.CanViewAsync(task.WorkspaceId, currentUser.UserId, LinkedResourceTypes.Task, taskId, ct))
        {
            throw new ForbiddenException("You do not have permission to access this task.");
        }

        var all = await comments.ListForTaskAsync(taskId, ct);
        return await BuildThreadsAsync(all, ct);
    }
    ...
    public async Task<CommentDto> EditAsync(Guid commentId, string body, CancellationToken ct = default)
    {
        ...
        return await ToDtoAsync(comment, Array.Empty<Comment>(), ct);
    }
    ...
    private async Task<IReadOnlyList<CommentDto>> BuildThreadsAsync(IReadOnlyList<Comment> all, CancellationToken ct)
    {
        var byAttachment = await LoadAttachmentsAsync(all.Select(c => c.Id).ToList(), ct);
        var byParent = all.Where(c => c.ParentId is not null).ToLookup(c => c.ParentId!.Value);
        return all
            .Where(c => c.ParentId is null)
            .OrderBy(c => c.CreatedAtUtc)
            .Select(c => ToDto(c, byParent[c.Id].OrderBy(r => r.CreatedAtUtc).ToList(), byAttachment))
            .ToList();
    }

    private async Task<CommentDto> ToDtoAsync(Comment c, IReadOnlyList<Comment> replies, CancellationToken ct)
    {
        var byAttachment = await LoadAttachmentsAsync([c.Id, .. replies.Select(r => r.Id)], ct);
        return ToDto(c, replies, byAttachment);
    }

    private async Task<ILookup<Guid, CommentAttachmentDto>> LoadAttachmentsAsync(IReadOnlyList<Guid> commentIds, CancellationToken ct)
    {
        var all = await attachments.ListForCommentsAsync(commentIds, ct);
        return all.ToLookup(a => a.CommentId, a => new CommentAttachmentDto(
            a.Id, a.CommentId, a.FileName, a.ContentType, a.SizeBytes, a.UploadedByUserId, a.CreatedAtUtc));
    }

    private static CommentDto ToDto(Comment c, IReadOnlyList<Comment> replies, ILookup<Guid, CommentAttachmentDto> attachmentsByComment) => new(
        ...
        replies.Select(r => ToDto(r, Array.Empty<Comment>(), attachmentsByComment)).ToList(),
        attachmentsByComment[c.Id].OrderBy(a => a.CreatedAtUtc).ToList());
}
