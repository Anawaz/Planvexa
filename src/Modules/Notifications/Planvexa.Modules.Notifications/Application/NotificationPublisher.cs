...
public sealed class NotificationPublisher(
    ...
    IRealtimeNotifier realtime) : INotificationPublisher
{
    ...
    public async Task PublishAsync(NotificationRequest request, CancellationToken cancellationToken = default)
    {
        ...
        // Workspace-wide group is the only realtime channel that exists (see WorkspaceHub); every
        // connection in the workspace gets this, so the entityId carries the recipient rather than the
        // notification so useRealtime.ts can filter to "was this event for me" before invalidating.
        await realtime.NotifyAsync(
            new RealtimeEvent(workspaceId, "Notification", request.RecipientUserId, "created", null, workspace.CorrelationId),
            cancellationToken);
    }
}
