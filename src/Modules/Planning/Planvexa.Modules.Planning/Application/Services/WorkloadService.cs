    ...
    : PlanningServiceBase(ctx)
{
    public async Task<IReadOnlyList<WorkloadRowDto>> ComputeAsync(DateTimeOffset fromUtc, DateTimeOffset toUtc, CancellationToken ct)
    {
        ...
        // Unassigned bucket (sentinel Guid.Empty, mirroring the Timeline view's client-side "Unassigned"
        // swimlane): open tasks nobody owns don't consume anyone's capacity, so "over-allocated" doesn't
        // apply -- surfaced purely so a manager can see and claim orphaned scheduled effort.
        var unassignedTaskIds = await work.UnassignedTaskIdsAsync(workspaceId, ct);
        if (unassignedTaskIds.Count > 0)
        {
            var unassignedEstimates = await estimates.ListByTaskIdsAsync(workspaceId, unassignedTaskIds, ct);
            var unassignedScheduledSeconds = unassignedEstimates.Sum(e => e.EstimateSeconds);
            rows.Add(new WorkloadRowDto(Guid.Empty, 0m, WorkloadEngine.Hours(unassignedScheduledSeconds), 0m, false));
        }
