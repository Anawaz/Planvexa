...
public sealed class InvitationService(
    ...
    IUnitOfWork unitOfWork) : IInvitationDirectoryQuery
{
    private static readonly TimeSpan InvitationValidity = TimeSpan.FromDays(14);

    /// <summary>Cross-module query (see <see cref="IInvitationDirectoryQuery"/>) — lets the Identity
    /// module check for a pending invitation when gating self-registration, without depending on
    /// Tenancy internals.</summary>
    public Task<bool> HasPendingInvitationAsync(string email, CancellationToken cancellationToken = default)
        => invitations.HasPendingForEmailAsync(email.Trim().ToLowerInvariant(), cancellationToken);
    ...
    /// </summary>
    public async Task<InvitationAcceptedDto> AcceptAsync(string rawToken, Guid userId, string userEmail, CancellationToken cancellationToken = default)
    {
        ...
        if (!string.Equals(invitation.Email, userEmail.Trim().ToLowerInvariant(), StringComparison.Ordinal))
        {
        ...
        if (existingMembership is not null)
        {
            ...
            await NotifyInviterAsync(invitation, userId, cancellationToken);
            await unitOfWork.SaveChangesAsync(cancellationToken);
    ...
    /// <summary>Lets the inviter know their invite was accepted. Deduped per invitation, so a retried
    /// accept call is a safe no-op. A self-invite (inviter accepting their own invitation, e.g. via an
    /// alternate identity) skips notifying — there is no one else to tell.</summary>
    private Task NotifyInviterAsync(Invitation invitation, Guid acceptedByUserId, CancellationToken cancellationToken)
    {
        if (invitation.InvitedByUserId == acceptedByUserId)
        {
        ...
        return notifications.PublishAsync(new NotificationRequest(
            ...
            WorkspaceId: invitation.WorkspaceId,
            DeduplicationKey: $"invitation.accepted:{invitation.Id:N}",
            Payload: new Dictionary<string, string> { ["byUserId"] = acceptedByUserId.ToString(), ["memberEmail"] = invitation.Email }), cancellationToken);
    }
