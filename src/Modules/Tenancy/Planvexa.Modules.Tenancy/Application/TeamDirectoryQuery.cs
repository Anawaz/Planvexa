...
public sealed class TeamDirectoryQuery(ITeamStore teams) : ITeamDirectoryQuery
{
    ...
    public async Task<bool> TeamExistsAsync(Guid workspaceId, Guid teamId, CancellationToken ct = default)
    {
        var team = await teams.FindAsync(teamId, ct);
        return team is not null && team.WorkspaceId == workspaceId;
    }
}
