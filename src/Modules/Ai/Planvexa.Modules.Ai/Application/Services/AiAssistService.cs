...
public sealed class AiAssistService(
    ...
    : AiServiceBase(ctx)
{
    /// <summary>The workspace's "allow AI to be completely disabled" master switch. No stored settings
    /// row means AI was never disabled.</summary>
    private async Task<bool> AiFeaturesEnabledAsync(Guid workspaceId, CancellationToken ct)
        => (await settingsStore.FindAsync(workspaceId, ct))?.AiFeaturesEnabled ?? true;

    /// <summary>See <see cref="AiCreditLimitGuard"/>: rejects a real-provider call once the workspace has spent
    /// its monthly token allowance, never the offline extractive fallback.</summary>
    private Task EnsureWithinCreditLimitAsync(Guid workspaceId, CancellationToken ct)
        => AiCreditLimitGuard.EnsureWithinLimitAsync(settingsStore, requests, workspaceId, Now, ct);
    ...
    public async Task<AiSummaryDto> SummarizeDocumentAsync(Guid documentId, string? idempotencyKey, CancellationToken ct)
    {
        ...
        AiAuthorizer.EnsureUse((await AccessAsync(workspaceId, ct))?.Role, await AiFeaturesEnabledAsync(workspaceId, ct));
        ...
        await EnsureWithinCreditLimitAsync(workspaceId, ct);
    ...
    public async Task<AiSummaryDto> SummarizeChatAsync(Guid channelId, string? idempotencyKey, CancellationToken ct)
    {
        ...
        AiAuthorizer.EnsureUse(role, await AiFeaturesEnabledAsync(workspaceId, ct));
    ...
    public async Task<AiUsageDto> GetUsageAsync(CancellationToken ct)
    {
        ...
        var creditLimit = (await settingsStore.FindAsync(workspaceId, ct))?.CreditLimit;
        return new AiUsageDto(count, tokens, creditLimit.HasValue, creditLimit);
    }
