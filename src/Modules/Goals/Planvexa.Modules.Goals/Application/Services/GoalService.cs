    ...
    : GoalServiceBase(ctx)
{
    ...
    public async Task<GoalDetailDto> GetDetailAsync(Guid id, CancellationToken ct)
    {
        ...
        var keyResultDtos = goal.KeyResults.Select(ToKeyResultDto).ToList();

        return new GoalDetailDto(dto, linkedDtos, keyResultDtos);
    }
    ...
    public async Task<GoalDto> CreateAsync(CreateGoalCommand command, CancellationToken ct)
    {
        ...
        var goal = Goal.Create(
            ...
            command.StartDate, command.EndDate, command.TargetType, command.TargetValue, command.CurrentValue, Now, command.Unit);
        goals.Add(goal);
    ...
    public async Task<GoalDto> LinkKeyResultAsync(Guid id, LinkKeyResultCommand command, CancellationToken ct)
    {
        var workspaceId = RequireWorkspace();
        GoalAuthorizer.EnsureEdit(await RoleAsync(workspaceId, ct));

        var goal = await goals.FindWithLinkedTasksAsync(workspaceId, id, ct) ?? throw new NotFoundException("Goal not found.");
        goal.LinkKeyResult(NewId(), command.Title, command.TargetValue, command.CurrentValue, command.Unit, Now);
        Audit("goals.key_result_added", "Goal", goal.Id, new { command.Title });
        await SaveAsync(ct);
        return await ToDtoAsync(workspaceId, goal, ct);
    }

    public async Task<GoalDto> UpdateKeyResultAsync(Guid id, Guid keyResultId, UpdateKeyResultCommand command, CancellationToken ct)
    {
        var workspaceId = RequireWorkspace();
        GoalAuthorizer.EnsureEdit(await RoleAsync(workspaceId, ct));

        var goal = await goals.FindWithLinkedTasksAsync(workspaceId, id, ct) ?? throw new NotFoundException("Goal not found.");
        goal.UpdateKeyResult(keyResultId, command.Title, command.CurrentValue, command.TargetValue, command.Unit, Now);
        Audit("goals.key_result_updated", "Goal", goal.Id, new { keyResultId });
        await SaveAsync(ct);
        return await ToDtoAsync(workspaceId, goal, ct);
    }

    public async Task<GoalDto> RemoveKeyResultAsync(Guid id, Guid keyResultId, CancellationToken ct)
    {
        var workspaceId = RequireWorkspace();
        GoalAuthorizer.EnsureEdit(await RoleAsync(workspaceId, ct));

        var goal = await goals.FindWithLinkedTasksAsync(workspaceId, id, ct) ?? throw new NotFoundException("Goal not found.");
        if (goal.RemoveKeyResult(keyResultId, Now))
        {
            Audit("goals.key_result_removed", "Goal", goal.Id, new { keyResultId });
            await SaveAsync(ct);
        }
    ...
    private static GoalKeyResultDto ToKeyResultDto(GoalKeyResult k)
        => new(k.Id, k.Title, k.CurrentValue, k.TargetValue, k.Unit, GoalProgressCalculator.NumericPercent(k.CurrentValue, k.TargetValue));
    ...
    private async Task<GoalDto> ToDtoAsync(Guid workspaceId, Goal goal, CancellationToken ct)
    {
        ...
        return new GoalDto(
            ...
            goal.TargetType, goal.TargetValue, goal.CurrentValue, goal.Unit, goal.Status, percent, total, completed, goal.KeyResults.Count);
    }
