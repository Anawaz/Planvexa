...
public interface IResourcePermissionQuery
{
    ...
    /// <summary>
    /// Bulk form of <see cref="HasAnyGrantAsync"/>: a single indexed query returning which of
    /// <paramref name="resourceIds"/> have at least one ACL row, instead of one round trip per id. Lets a
    /// caller listing many resources of the same type (e.g. every Task in a List) batch the cheap
    /// existence pre-check down to one query and only run the full resolver-based
    /// <see cref="GetEffectiveAsync(Guid,Guid,string,Guid,CancellationToken)"/> walk for the (typically
    /// small) subset that actually needs it.
    /// </summary>
