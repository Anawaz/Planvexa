...
public interface ITaskWriteApi
{
    ...
    /// <summary>Removes a tag by (case-insensitive) name from the task. Idempotent no-op (returns true,
    /// no write) if the workspace has no such tag or the task doesn't have it. False if the task is missing.</summary>
    Task<bool> RemoveTagByNameAsync(Guid taskId, string tagName, CancellationToken cancellationToken = default);
