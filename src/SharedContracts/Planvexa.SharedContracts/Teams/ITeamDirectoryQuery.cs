...
public interface ITeamDirectoryQuery
{
    ...
    /// <summary>True only if the Team exists AND belongs to this Workspace — the reference-integrity
    /// check other modules must run before persisting a caller-supplied Team id (a syntactically valid
    /// GUID from another Workspace must never be accepted).</summary>
    Task<bool> TeamExistsAsync(Guid workspaceId, Guid teamId, CancellationToken ct = default);
}
